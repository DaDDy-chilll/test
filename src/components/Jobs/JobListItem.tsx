...
const JobListItem = ({item, setShowDetails}: JobListItemProps) => {
  ...
  return (
    <div className='flex justify-start items-center gap-4 w-full cursor-pointer' onClick={handleShowDetails}>
      <div className='w-20 h-2w-20 rounded-md'>
        <img src={item.img} alt={item.position} className='w-full h-full object-cover rounded-md' />
      </div>
      <div className='flex justify-between items-center w-full'>
        <div className='flex flex-col gap-1'>
            <h1 className='text-md font-semibold'>{item.position}</h1>
            <p className='text-xs text-gray-500'>{item.location}</p>
            <p className='text-sm text-gray-700'>{item.applied} persons applied</p>
        </div>
